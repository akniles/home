...
public:
	...
	void SiftDown(size_t index, vector<size_t>* indexes) {
		...
		size_t more = index;
		if (left < size_ && (heap_[left].number > heap_[index].number ||
			(heap_[left].number == heap_[index].number
				&& heap_[left].start < heap_[index].start))) {
			more = left;
		}
		if (right < size_ && (heap_[right].number > heap_[index].number ||
			(heap_[right].number == heap_[index].number &&
	...
	void Insert(HeapValue value, vector<size_t>* indexes, size_t position) {
		...
		while (index != 0 && (heap_[(index - 1) / 2].number < heap_[index].number ||
			(heap_[(index - 1) / 2].number == heap_[index].number &&
	...
	void SiftUp(size_t index, vector<size_t>* indexes) {
		...
		if (heap_[previous].number < heap_[index].number ||
			(heap_[previous].number == heap_[index].number &&
...
void Free(MaxHeap* sectors, FixedSet<Query>* set, FixedSet<Edge>* ends, FixedSet<Edge>* starts,
	...
	if (ends->Contains(deleted.start - 1) && starts->Contains(deleted.end + 1)) {
...
void Process(const Data& data, vector<int>* result) {
	...
	for (size_t index = 0; index < data.sequence.size(); ++index) {
		if (data.sequence[index] > 0) {
			if (!sectors.empty() && sectors.GetMax() >= data.sequence[index]) {
	...
	struct CustomLess {
		inline bool operator()(const Edge& one, const Edge& other) {
			return (one.value < other.value ||
				(one.value == other.value && one.start > other.start));
		}
	};
...
template<class T>
void PrintData(const T& sequence) {
	std::cout << sequence.size << "\n";
	for (const auto& element : sequence.sequence) {
		std::cout << element << ' ';
	}
	std::cout << '\n';
}
...
void StressTest(Data RandomData(int seed),
	...
	void PrintData(const Data&), int iterations) {
...
int main() {
	StressTest(RandomData, Process, TrueAnswer, Check, PrintArray, PrintData, 10000);
	return 0;
}
